...
// openid → get an ID token; email → the claim the API's auth decorator requires.
 ...
 */
class FlowOAuthException(val code: String, val detail: String?) :
    Exception("OAuth failed: $code" + (detail?.let { " ($it)" } ?: ""))

@Serializable
internal data class FlowTokenEndpointResponse(
...
internal data class FlowCallbackParams(
    val code: String?,
    val state: String?,
    val error: String?,
...
internal fun buildAuthUrl(
  ...
  // Optional hint so the browser pre-selects a Workspace account in the allowed domain.
  if (hostedDomain != null) params["hd"] = hostedDomain
  return authEndpoint +
      "?" +
...
internal fun toFlowTokenSet(response: FlowTokenEndpointResponse): FlowTokenSet {
  val idToken = response.idToken ?: throw FlowOAuthException("no_id_token", null)
  val expiry =
      FlowJwt.expiresAtEpochSec(idToken)
          ?: (System.currentTimeMillis() / 1000 + (response.expiresIn ?: 3600))
  return FlowTokenSet(idToken, response.refreshToken, expiry)
}
 ...
 * `127.0.0.1:<port>`).
 */
internal class FlowLoopbackReceiver : AutoCloseable {
  private val server = HttpServer.create(InetSocketAddress(InetAddress.getLoopbackAddress(), 0), 0)
  private val received = ArrayBlockingQueue<FlowCallbackParams>(1)
  ...
  init {
    server.createContext("/") { exchange ->
      ...
      val body =
          "<html><body style=\"font-family:sans-serif\">Signed in to Flow. You can close this tab " +
              "and return to the terminal.</body></html>"
  ...
  fun awaitCallback(timeout: Duration): FlowCallbackParams =
      received.poll(timeout.toMillis(), TimeUnit.MILLISECONDS)
          ?: throw FlowOAuthException("timeout", "Timed out waiting for the browser sign-in.")
...
/** Drives the human OAuth sign-in and token refresh for the read-only Flow commands. */
class FlowOAuth(
    private val clientId: String,
    private val clientSecret: String,
    private val hostedDomain: String? = null,
   ...
   */
  fun login(
      openBrowser: (String) -> Boolean = ::openInBrowser,
      echo: (String) -> Unit,
  ): FlowTokenSet {
    val verifier = generateCodeVerifier()
    val state = randomUrlSafe(16)
    FlowLoopbackReceiver().use { receiver ->
      val url =
          buildAuthUrl(clientId, receiver.redirectUri, codeChallenge(verifier), state, hostedDomain)
      ...
      if (callback.error != null)
          throw FlowOAuthException(callback.error, callback.errorDescription)
      if (callback.state != state) {
        throw FlowOAuthException("state_mismatch", "OAuth state did not match; aborting.")
      }
      val code =
          callback.code ?: throw FlowOAuthException("no_code", "No authorization code returned.")
      return exchangeCode(code, verifier, receiver.redirectUri)
    }
  }

  private fun exchangeCode(code: String, verifier: String, redirectUri: String): FlowTokenSet =
      tokenRequest(
          mapOf(
              "client_id" to clientId,
  ...
  fun refresh(refreshToken: String): FlowTokenSet =
      tokenRequest(
          mapOf(
              "client_id" to clientId,
              "client_secret" to clientSecret,
  ...
  private fun tokenRequest(form: Map<String, String>): FlowTokenSet {
    ...
    val parsed = oauthJson.decodeFromString<FlowTokenEndpointResponse>(response.body())
    if (response.statusCode() !in 200..299 || parsed.idToken == null) {
      throw FlowOAuthException(
          parsed.error ?: "http_${response.statusCode()}",
          parsed.errorDescription,
      )
    }
    return toFlowTokenSet(parsed)
  }
}
