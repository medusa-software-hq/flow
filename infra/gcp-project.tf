...
resource "google_project" "gcp_project" {
  ...
  # Per environment: "flow - baseline" on prod (suffix empty — the existing project
  # must not be renamed), "flow - baseline - staging" on the staging workspace. The
  # project_id keeps its random suffix; each workspace has its own state, hence its
  # own random_id, hence a distinct project.
