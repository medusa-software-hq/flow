...
import { KeyCodes } from '../../utils/KeyCodes.ts';
import {
  type MyEdge,
  type MyNode,
  myNodeTypes,
...
import type { CSessionEditor } from '../../session_editor/CSessionEditor.ts';
import { type TaskNode, taskNodeTag } from './TaskNode.tsx';
...
export function SessionCanvas(props: SessionCanvasProps) {
  ...
  const selectedNodeIdsSnap: ReadonlySet<string> =
    useSnapshot(selectedNodeIdsLive);
  ...
  const selectedEdgeIdsSnap: ReadonlySet<string> =
    useSnapshot(selectedEdgeIdsLive);
      ...
      if (connectionState.isValid) {
        ...
        const fromTaskId = (() => {
          switch (fromNodeType) {
            ...
            default:
              throw new Error(
                `Unknown source node type: ${String(fromNodeType)}`,
              );
          }
        })();
      ...
      for (const change of changes) {
        switch (change.type) {
          case 'remove': {
            ...
            const removedEdgeData = edges.find(
              (edge) => edge.id === removedEdgeId,
            )?.data;
