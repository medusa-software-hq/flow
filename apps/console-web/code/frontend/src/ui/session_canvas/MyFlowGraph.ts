...
import { TaskNode, taskNodeTag } from './TaskNode.tsx';
...
export function useMyFlowGraph(
  args: UseMyFlowGraphArgs,
): readonly [MyNode[], MyEdge[]] {
  ...
  for (const [taskId, taskSnap] of sessionSourceSnap.taskById.entries()) {
    ...
    taskSnap._sourceTaskIds.forEach((sourceTaskId) => {
      ...
      if (sourceTaskSnap === undefined) {
        throw new Error(
          `Source task with ID ${String(sourceTaskId)} not found`,
        );
      }
