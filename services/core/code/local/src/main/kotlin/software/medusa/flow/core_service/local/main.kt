...
import software.medusa.flow.core_service.job_queue.InMemoryFlowJobQueue
...
suspend fun main() = coroutineScope {
  ...
  val inMemorySessionExecutionJobQueue = InMemoryFlowJobQueue()
    ...
    runControlService(
        configurator =
            ControlConfigurator(
                ...
                flowJobQueueFront = inMemorySessionExecutionJobQueue,
            ),
    )
  }
  ...
  val workerDeferred = async {
    runWorker(
        configurator =
            WorkerConfigurator(
                flowDatabase = flowDatabase,
                flowJobQueueBack = inMemorySessionExecutionJobQueue,
            ),
    )
  }
